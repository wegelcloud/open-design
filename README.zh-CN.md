...
| **视觉方向** | 5 套精选流派（Editorial Monocle · Modern Minimal · Warm Soft · Tech Utility · Brutalist Experimental），每套自带 OKLch 色板 + 字体栈（[`packages/contracts/src/prompts/directions.ts`](packages/contracts/src/prompts/directions.ts)） |
...
下面整套机制都是 [`huashu-design`](https://github.com/alchaincyf/huashu-design) 的 playbook，被移植进 OD 的提示词栈，并通过 skill 副文件 pre-flight 让每个 skill 都能落地执行。看 [`packages/contracts/src/prompts/discovery.ts`](packages/contracts/src/prompts/discovery.ts) 是真实文案：
