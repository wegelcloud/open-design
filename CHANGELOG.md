...
A maintainability-and-extension release: Critique Theater advances to **Phase 7** (reducer + stream/replay hooks) with a daemon-side **Phase 6.2** artifact extraction endpoint, a new **auto-memory store** captures agent context across runs, HyperFrames lands **HTML-in-Canvas** end-to-end, and a **thumbs-up/down feedback widget** ships under completed assistant turns. The skill model gets a **major refactor** (generic skills + design-templates split + finalize-design API). A new **`tools-pr` maintainer workspace** plus `MAINTAINERS.md` formalize PR-duty operations. New BYOK plumbing (fetch models button, alphabetical sort), **HTTP 206 range requests** for video/audio, **scheduled routines** for unattended agent runs, **macOS Intel (x64) builds**, an official **Nix flake**, four new design systems (hud, loom, trading-terminal, WeChat), and an `agent-browser` skill. 97 merged PRs since 0.6.0.
...
- **`tools-pr` maintainer PR-duty workspace.** New tooling for triaging, reviewing, and merging community PRs at scale. ([#1259])
- **`MAINTAINERS.md` + CONTRIBUTING entry-point** covering maintainer rules. ([#1290])
- **Contributor card bot workflow.** ([#932])
- Enforce PR→issue linking discipline. ([#1263])
...
#### Maintainer tooling
- `tools-pr`: fall back on `reviewDecision` for unresolved-changes-requested. ([#1287])
- `tools-pr`: chunk stats fetch through cursor-paginated GraphQL. ([#1285])
- Map `pull_request_target` to contributor bot event. ([#1092])
